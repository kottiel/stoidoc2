...
int print_label_idoc_records(FILE *fpout, Label_record *labels, int record, Ctrl *idoc) {
    ...
    if ((labels[record].size) && (!equals_no(graphic_val))) {
    ...
    if ((labels[record].level) && (!equals_no(labels[record].level))) {
    ...
    char *endptr;
    if ((labels[record].barcodetext) && (!equals_no(labels[record].barcodetext))) {
...
int main(int argc, char *argv[]) {
   ...
   // Column_header columns = {0};
    ...
    strcat(outputfile, "_IDoc (stoidoc2).txt");
